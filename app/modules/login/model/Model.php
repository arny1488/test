...
class LoginModel extends Model
{
    ...
    public function doPassChange(string $email, string $hash, string $pass): bool
    {
        ...
        return DB::update('users', ['password' => $password_hash, 'salt' => $salt, 'hash' => '', 'hash_expire' => null], DB::statement()->with('email = ?', $email)->andWith('hash = ?', $hash)->andWith('hash_expire >= ?', date(DB_DATETIME_FORMAT))) > 0;
