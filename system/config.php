...
const APP_NAME = 'TM4RENT';
const APP_VERSION = '2.0';

define('APP_BUILD', APP_VERSION . '.' . trim(@file_get_contents(DASHBOARD_DIR . '/VERSION') ?: date('U')));
...
if (file_exists(DASHBOARD_DIR . '/configs/environment.php')) {
    include_once(DASHBOARD_DIR . '/configs/environment.php');
}
...
if (!defined('TIMEZONE')) {
    $browser_timezone = $_SERVER['HTTP_TIMEZONE'] ?? ($_COOKIE["browser_timezone"] ?? null);
    define(
        'TIMEZONE',
        ($browser_timezone && Core::isValidTimezoneId($browser_timezone)) ? $browser_timezone : (defined('TIMEZONE') ? DEFAULT_TIMEZONE : 'Europe/Moscow')
    );
}
