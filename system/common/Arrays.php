...
class Arrays
{
    ...
    public static function deleteKey(array &$array, string $key_name): void
    {
        ...
        $_isKey = function (&$value, $key, $name): bool
        {
            return ($key === $name);
        };

        $array_walk_recursive_delete = function (array &$array, callable $callback, $key_name = null) use (&$array_walk_recursive_delete): array
        {
            foreach ($array as $key => &$value) {
                if (is_array($value)) {
    ...
    public static function deleteKeyValue(array &$array, string $key_name, string $val): void
    {
        ...
        $_isKeyVal = function ($value, $key, $name, $val): bool
        {
            return ($key === $name) && ($value === $val);
        };

        $array_walk_recursive_delete = function (array &$array, callable $callback, $key_name = null, $val = null) use (&$array_walk_recursive_delete): array
        {
            foreach ($array as $key => &$value) {
                if (is_array($value)) {
     ...
     * @return bool|int|string
     */
    public static function pathByValue(array $array, mixed $val, string $glue = '.', bool $add_self_key = true, array &$stack = []): bool|int|string
    {
        ...
        foreach ($array as $k => $v) {
            ...
            if (is_array($v) && self::pathByValue($v, $val, $glue, $add_self_key, $stack)) {
                array_unshift($stack, $k);
                return implode($glue, $stack);
            }
        }
