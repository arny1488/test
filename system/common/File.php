...
class File
{
    ...
    private static function _delete(string $path): void
    {
     ...
     * @return string
     */
    public static function hash(string $filename, string $algo = 'md5'): string
    {
    ...
    public static function hashPath(string $filename, bool $dir_only = false): string
    {
        return (strlen($filename) > 6)
            ? substr($filename, 0, 2) . '/' . substr($filename, 2, 2) . '/' . substr($filename, 4, 2) . ($dir_only ? '' : '/' . $filename)
            : ($dir_only ? '' : '/' . $filename);
    }
