...
class Navigation
{
     ...
     */
    public static function add(int $sorting, string $name, string $icon, string $url, string $html, string $id, int $type, int $rubric, string $parent, string $onclick, bool $external = false): void
    {
        self::$items[] = [
            'sorting' => $sorting,
            'name' => $name,
    ...
    public static function get(int $type): array
    {
        ...
        foreach (self::$items as $nav) {
            ...
            if ($nav['parent']) {
                ...
                $nav_sort[$nav['type']][$nav['rubric']][$nav['id']]['html'] = $nav['html'];
