...
class DB
{
    ...
    private function __construct(array $config)
    {
        ...
        $connection_string = sprintf(
            "%s:host=%s;port=%d;dbname=%s;user=%s;password=%s;charset=utf8;",
     ...
     * @return array
     */
    private static function cache_handler(string $method, string $statement, ...$params): mixed
    {
        if ((self::$cache_TTL > 0) && (str_starts_with(strtoupper(trim($statement)), 'SELECT') || str_starts_with(strtoupper(trim($statement)), 'WITH RECURSIVE'))) {
            $matches = null;
            preg_match('/FROM\s+([^\s]+)/', strtoupper($statement), $matches);
            ...
            $cache_dir = DASHBOARD_DIR . TEMP_DIR . '/cache/sql/' . $table . File::hashPath($cache_file, true) . '/';

            Dir::create($cache_dir);

            if (!(file_exists($cache_dir . $cache_file) && (time() - @filemtime($cache_dir . $cache_file) < self::$cache_TTL))) {
                $result = self::$driver->$method($statement, ...$params);
        ...
        return self::$driver->$method($statement, ...$params);
    }
    ...
    {
        return self::cache_handler('run', $statement, ...$params);
    }

    /**
     * Perform a Query (alias of EasyDB->row)
     *
     * @param string $statement
     * @param mixed ...$params
     * @return array
     */
    public static function row(string $statement, ...$params): array
    {
        return self::cache_handler('row', $statement, ...$params);
    }

    /**
     * Perform a Query (alias of EasyDB->cell)
     *
     * @param string $statement
     * @param mixed ...$params
     * @return array
     */
    public static function cell(string $statement, ...$params): float|bool|int|string|null
    {
        return self::cache_handler('cell', $statement, ...$params);
    }

    public static function clearCache(?string $table): void
    {
        Dir::delete_contents(DASHBOARD_DIR . TEMP_DIR . '/cache/sql' . ($table ? '/' . $table : ''));
    }
    ...
    public static function backup(): bool // ToDo: refactor
    {
        ...
        Log::log($res ? Log::INFO : Log::ERROR, 'DB', $res ? "Database backup created in $file" : 'Database backup error');
    ...
    public static function buildSearch(array $fields, string $search): string
    {
        ...
        if ($search) {
            $chunks = explode(' ', preg_replace('/\s+/', ' ', trim(self::escapeLikeValue($search))));
